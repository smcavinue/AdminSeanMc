...
function RefreshEXOSession {
    ...
    if ($session.state -ne "opened") {
        ...
        Connect-ExchangeOnline -Certificate $Certificate -AppID $clientid -Organization ($orgdetails.verifieddomains | ? { $_.isinitial -eq "true" }).name
    }
}
...
write-host "number 1" -ForegroundColor yellow
##Get M365 Client Apps usage Report
$apiUri = "https://graph.microsoft.com/beta/reports/getM365AppUserDetail(period='D30')"
$ClientAppUsage = ((Invoke-RestMethod -Headers @{Authorization = "Bearer $($Token.accesstoken)" } -Uri $apiUri -Method Get) | ConvertFrom-Csv)
...
foreach ($room in $RoomMailboxes) {
    RefreshEXOSession
    $RoomStats = $null
    $RoomStats = get-mailboxstatistics $room.primarysmtpaddress
...
foreach ($equipment in $EquipmentMailboxes) {
    RefreshEXOSession
    $EquipmentStats = $null
    $EquipmentStats = get-mailboxstatistics $equipment.primarysmtpaddress
...
foreach ($SharedMailbox in $SharedMailboxes) {
    RefreshEXOSession
    $SharedStats = $null
    $SharedStats = get-mailboxstatistics $SharedMailbox.primarysmtpaddress
...
foreach ($user in ($users | ? { $_.mail -ne $null })) {
    RefreshEXOSession
    $stats = $null
    $stats = get-mailboxstatistics $user.mail -erroraction SilentlyContinue
...
foreach ($domain in $orgdetails.verifieddomains) {
    Try {
        [array]$MXRecords = Resolve-DnsName -Name $domain.name -Type mx
    }
    catch {
...
Try {
    ...
    $M365AppsUsage  | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "M365 Apps Usage Usage" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow
    ##Export Client Apps Usage Report
    $ClientAppUsage  | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "Client App Usage" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow
...
###########POSTFIX AND FORMATTING##################
$ProgressStatus = "Applying report formatting..."
UpdateProgress
$ProgressTracker++
