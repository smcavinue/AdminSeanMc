...
##          Returns a report of Sharepoint file and folder structure to CSV file
...
function OutputFileReport {
   ...
   #>
    ...
    [PSCustomObject]$Export | Export-CSV $outputfilePath -NoClobber -NoTypeInformation -Append -Force
...
foreach ($item in $items) {
    Write-Progress -Activity "Performing File Report" -Status "Processing Item $($item.name): $tracker / $($items.count)" -PercentComplete (($tracker / ($items.count)) * 100)
    $tracker++
