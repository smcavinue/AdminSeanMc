...
foreach ($Deployment in $Deployments) {
    ...
    If ($Deployment.fields.DistributeTo -eq "Everyone except external users") {
        write-output "Deployed to EEEU Group"
    ...
    elseif ($User.fields.ContentType -eq "Person") {
        write-output "Deployed to User: $($User.fields.username)"
        Try{
            ...
            write-output "User $($User.fields.username) does not have a OneDrive, skipping deployment"
        }
    }
    elseIf ($User.fields.ContentType -eq "DomainGroup") {
        write-output "Deployed to Group: $($Deployment.fields.DistributeTo)"
        ...
        foreach($GroupMember in $GroupMembers) {
            Try{
                ...
                write-output "Deploying to User: $GroupMember via Group: $($Deployment.fields.DistributeTo)"
                ...
                write-output "User $GroupMember does not have a OneDrive, skipping deployment"
            }
        }
