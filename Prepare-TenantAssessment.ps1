...
##          Please fully read and test any scripts before running in your production environment!
    ...
    # May the force 2023: ask:ok, modified $appName, -CertificatePassword, -certificatename
...
function New-AadApplicationCertificate {
    ...
    try {
        $cert = (New-SelfSignedCertificate -Subject "ask:us Tenant Assessment" -friendlyName "ask:us Azure App Certificate for Tenant Assessment"-DnsName "TenantAssessment.askus.biz" -CertStoreLocation "cert:\currentuser\My" -KeyExportPolicy Exportable -Provider "Microsoft Enhanced RSA and AES Cryptographic Provider" -NotAfter $notAfter)
...
$connected = $false
##Name of the app
$appName = "ask:us Tenant Assessment Tool"
...
##Tenant ID
$TenantID = $null
##Certificate / App Password
$appPassword = Get-Credential -Username "CertificatePassword" -Message "Enter password for certificate and note it on a secure location"
...
$Thumbprint = New-AadApplicationCertificate -ClientId $appReg.AppId -CertificatePassword $appPassword -AddToApplication -certificatename "ask:us Tenant Assessment Certificate"
